...
import { PreKeySignalMessage, SignalMessage } from "./WhisperTextProtocol";
...
export class SessionCipher {
  ...
  public async encrypt(
    ...
    return await this.queueJob(async () => {
      ...
      const keys = await crypto.deriveSecrets(
      ...
      msg.ciphertext = await crypto.encrypt(
        keys[0],
        data,
        keys[2].slice(0, 16)
      );
      const msgBuf = SignalMessage.encode(msg).finish();
      ...
      if (session.pendingPreKey) {
        ...
        const preKeyMsg = PreKeySignalMessage.create({
        ...
        body = Buffer.concat([
          ...
          Buffer.from(PreKeySignalMessage.encode(preKeyMsg).finish()),
        ]);
      } else {
        type = 1; // normal
        body = result;
  ...
  public async decryptPreKeyWhisperMessage(
    ...
    return await this.queueJob(async () => {
      ...
      const preKeyProto = PreKeySignalMessage.decode(data.slice(1));
      if (!record) {
  ...
  private async doDecryptWhisperMessage(
    ...
    const message = SignalMessage.decode(messageProto);
  ...
  private async fillMessageKeys(chain: any, counter: number): Promise<void> {
    ...
    chain.messageKeys[chain.chainKey.counter + 1] = await crypto.calculateMAC(
      key,
      Buffer.from([1])
    );
  ...
  private async calculateRatchet(
    ...
    const masterKey = await crypto.deriveSecrets(
      sharedSecret,
      ratchet.rootKey,
